...
export async function executeTool(name,args,agent,repo,drafts){
 switch(name){
...
export async function runAgent(repo,user,agentId,input,fetcher=fetch,options={}){
 ...
 const allowed=definitions.filter(t=>t.name!=='analyze_costs'||['finance','executive'].includes(agent.room));
 for(let turn=0;turn<4;turn++){
  ...
  for(const call of m.tool_calls.slice(0,8)){let result;try{result=await executeTool(call.function.name,JSON.parse(call.function.arguments),agent,repo,drafts);}catch{result={error:'Parâmetros da ferramenta inválidos.'};}messages.push({role:'tool',tool_call_id:call.id,content:JSON.stringify(result).slice(0,24000)});}
 }
